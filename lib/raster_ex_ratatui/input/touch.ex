defmodule RasterExRatatui.Input.Touch do
  ...
  Only the first contact counts: multitouch slots other than `0` are ignored, so a second finger never moves the cursor. Both multitouch type B (`abs_mt_slot`, `abs_mt_tracking_id`, `abs_mt_position_x/y`, what the Goodix controller of the Raspberry Pi Touch Display 2 sends) and single-touch panels (`abs_x/y` with `btn_touch`) are understood.

  Mouse events are emitted at the end of each evdev frame. [`input_event`](https://hex.pm/packages/input_event) delivers one frame per message and drops the `syn_report` that ends it, so `translate_all/2` takes the end of its list as the end of a frame; an explicit `syn_report` ends one too, for sources that keep it.
  ...
  A 24×16 panel of 6×8 cells; a tap on pixel (13, 9) is cell (2, 1).
      ...
      iex> # The two messages input_event sends for a tap on a Goodix panel.
      iex> {touch, [down]} = Touch.translate_all(touch, [
      ...>   {:ev_abs, :abs_mt_tracking_id, 33}, {:ev_abs, :abs_mt_position_x, 13},
      ...>   {:ev_abs, :abs_mt_position_y, 9}, {:ev_abs, :abs_mt_touch_major, 46},
      ...>   {:ev_key, :btn_touch, 1}, {:ev_abs, :abs_x, 13}, {:ev_abs, :abs_y, 9}
      ...> ])
      iex> {_touch, [up]} = Touch.translate_all(touch, [{:ev_abs, :abs_mt_tracking_id, -1}, {:ev_key, :btn_touch, 0}])
      iex> [{down.kind, down.x, down.y}, {up.kind, up.x, up.y}]
      [{"down", 2, 1}, {"up", 2, 1}]
  """
  ...
  @doc """
  Folds one evdev event, `{type, code, value}`, and returns the mouse events it completes: only a `syn_report` ends a frame here. For `input_event`'s messages, which carry no `syn_report`, use `translate_all/2`.
  ...
  @doc """
  Folds one frame of events, as `input_event` delivers it, or the `:disconnect` it sends when the panel goes away, which ends a contact in progress with an `"up"`.

  The end of the list ends the frame. A list that carries its own `syn_report`s is split at them instead, and nothing more is emitted after the last one.
  ...
  A finger landing on one cell and sliding to the next; the second finger (slot 1) changes nothing.
      ...
      iex> alias RasterExRatatui.Input.Touch
      iex> touch = Touch.new(size: {24, 16}, cell_at: fn {x, y} -> {div(x, 6), div(y, 8)} end)
      iex> {touch, [down]} = Touch.translate_all(touch, [{:ev_abs, :abs_mt_tracking_id, 1}, {:ev_abs, :abs_mt_position_x, 2}, {:ev_abs, :abs_mt_position_y, 2}])
      iex> {touch, []} = Touch.translate_all(touch, [{:ev_abs, :abs_mt_position_x, 4}])
      iex> {touch, []} = Touch.translate_all(touch, [{:ev_abs, :abs_mt_slot, 1}, {:ev_abs, :abs_mt_tracking_id, 2}, {:ev_abs, :abs_mt_position_x, 20}, {:ev_abs, :abs_mt_position_y, 12}])
      iex> {touch, [drag]} = Touch.translate_all(touch, [{:ev_abs, :abs_mt_slot, 0}, {:ev_abs, :abs_mt_position_x, 9}])
  ...
  A finger in the margin is ignored until it enters a cell.
      ...
      iex> {touch, []} = Touch.translate_all(touch, [{:ev_key, :btn_touch, 1}, {:ev_abs, :abs_x, 20}, {:ev_abs, :abs_y, 0}])
      iex> {_touch, [down]} = Touch.translate_all(touch, [{:ev_abs, :abs_x, 7}])
  ...
  def translate_all(%__MODULE__{} = t, events) when is_list(events) do
    ...
    # input_event drops the syn_report: the end of its list is the frame's.
    {t, reversed} =
      case List.last(events) do
        ...
        _unterminated ->
          {t, emitted} = report(t)
          {t, Enum.reverse(emitted, reversed)}
      end
  ...
  # The end of a frame: the contact ended, or it is somewhere.

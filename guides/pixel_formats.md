...
Two callbacks are optional. `c:RasterExRatatui.PixelFormat.rgb_row/4` packs a whole row of region pixels at once, the fast path for pixel regions. `c:RasterExRatatui.PixelFormat.unpack_row/2` turns packed pixels back into RGB8, which `RasterExRatatui.Raster.to_png/1` needs to snapshot the panel; for the format above it is `for <<v <- row>>, into: <<>>, do: <<v * 17, v * 17, v * 17>>`.
...
Region pixels are the expensive path. By default the raster calls `rgb_pixel/6` once per panel pixel a region covers, and the call itself is most of the cost, so a format should also implement the optional `c:RasterExRatatui.PixelFormat.rgb_row/4`, which packs a whole row of RGB8 pixels in one pass (a binary comprehension for a format that ignores the position, as the built-in ones do; a small recursive loop when it dithers). The two must produce the same bytes; `RasterExRatatui.PixelFormat.rgb_row/5` is the fallback and a handy way to compare them in a test. On a Raspberry Pi 4 the row path made a 696×480 region about twice as fast. ex_ratatui caps region bitmaps at 1280 px on the long side and the raster scales them nearest-neighbour onto larger rects, so the cost is bounded by the rect on the panel, not the scene.
